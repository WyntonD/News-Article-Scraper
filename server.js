...
app.get("/", function(req, res) {
	Article.find({}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "There's nothing scraped yet. Please click \"Scrape For Newest Articles\" for updated world news."});
		}
		else{
...
app.get("/scrape", function(req, res) {
	request("https://www.cnn.com/world", function(error, response, html) {
		...
		$("h3.cd__headline").each(function(i, element) {
			...
			var title = $(element).find("h3.cd__headline").text().trim();
			var summary = $(element).find("span.cd__headline-text").text().trim();
			var img = $(element).parent().parent().find("div.media").find("img").attr("src");
			result.link = link;
			result.title = title;
			if (summary) {
...
app.get("/saved", function(req, res) {
	Article.find({issaved: true}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "You have not saved any articles yet. Click \"Save Article\" to save news of your choice!" });
		}
		else {
